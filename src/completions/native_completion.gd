...
func _singleton_ready() -> void:
	if not ClassDB.class_exists(&"GDScriptLanguageService") or not ResourceLoader.exists(SERVICE_PATH):
		return
	_owner = load(SERVICE_PATH).get_instance()
	if not is_instance_valid(_owner):
		return
	_service = _owner.native
	_apply_configuration()
	EditorInterface.get_script_editor().script_close.connect(_on_script_close)
...
func _on_code_completion_requested(script_editor: CodeEdit) -> bool:
	if mode == "disabled" or not is_instance_valid(_service):
		return false
	var script := get_current_script() as Script
	if not is_instance_valid(script) or script.resource_path.is_empty():
		return false
	var id := script_editor.get_instance_id()
	if not _managers.has(id):
		_managers[id] = load(MANAGER_PATH).new()
	var manager = _managers[id]
	manager.attach(script_editor, script.resource_path)
	manager.parse_text()
	var uri: String = manager.get_uri()
	var line := script_editor.get_caret_line()
	var column: int = load(SERVICE_PATH).utf16_column(script_editor.get_line(line), script_editor.get_caret_column())

	var profile := "full" if mode == "replace" else "helpers"
	var result: Dictionary = _service.completion_ex(uri, line,
...
func _on_script_close(script: Script) -> void:
	for id in _managers.keys():
		var manager = _managers[id]
		if manager._script_path == script.resource_path:
			manager.detach()
			_managers.erase(id)
...
func clean_up() -> void:
	...
	if is_instance_valid(editor) and editor.script_close.is_connected(_on_script_close):
		editor.script_close.disconnect(_on_script_close)
